			...
			<div class="col-12">
				<div class="card">
                    <div class="mb-3 mt-3 d-flex justify-content-between align-items-center col-12">
                        <a href="{{route('quan-tri.product.create')}}"><button type="button" class="btn btn-info"><i class="fas fa-plus mr-2"></i>Thêm Mới</button></a>

                        <div class="card-tools">
                            <form class="form-inline ml-3" method="GET" action="{{route('admin.product.search')}}">
						...
						<table class="table table-hover text-nowrap">
							...
							<tbody>
								...
								<tr class="text-center">
									...
									<td>{{$item->name}}</td>
                                    <td>@if ($item->image)
                                            <img src="{{asset($item->image)}}" alt="{{$item->name}}" width="60" height="50">
                                        @endif
                                    </td>
									<td>{{$item->stock}}</td>
									...
									<td>{{ ($item->is_hot == 1) ? 'Nổi Bật' : 'Thường' }}</td>
                                    <td>{{$item->categories->name ?? 'Trống'}}</td>
